...
;Variables globales
	...
	DB "AH=12H PARA CONVERTIR CADENA INTRODUCIDA POR TECLADO A HEXADECIMAL"
	DB "AH=14H PARA IMPRIMRI UNA CADENA DECIMAL SIN CONVERSION"
...
DIV10h:
	...
	MOV AH,02H
IMPRh:
	CALL retraso;ESTA ASI PARA COMPROBAR QUE FUNCIONA PERO COMO NO FUNCIONA LA INT 1CH DEJAMOS LA LLAMADA
	DEC DI
	MOV DL, RESUL[DI]
	CMP DL,9
	JBE numeroh
...
nhex:
	...
	cmp al,'9'
	jbe numero  	;CUANDO AL SER >= QUE CERO O <= QUE 9 SALTA
	cmp al,'A'
...
;==================== AH = 14h ===============
; ========= Imprime cadena decimal en decimal =====================
; Parametros
; DX: offset inicial de la cadena con respecto a DS
; Retorna
; NADA
;==========================================================
imprimirCAD proc near
	PUSH SI BX CX DX DS DI
	MOV SI,DX
	CALL atoi  
	CALL imprimirD
...
; RETRASO DE 1 SEG
;===============================
retraso proc near ;SERIA FAR SI FUERA PARA LA INT 1CH
	;PUSHF
	PUSH AX BX CX DX
	XOR AX, AX
	;cli
...
instalar proc near
	...
	MOV ES:[60H*4+2],CS
	STI
	;MOV AX,ES:[1CH*4];NO FUNCINA POR ESO ESTA COMENTADO
	;MOV OLD_1C, AX
	;MOV AX,ES:[1CH*4+2]
	;MOV OLD_1C+2, AX
	;MOV AX,OFFSET retraso
	;CLI
	;MOV ES:[1CH*4],AX
	;MOV ES:[1CH*4+2],CS
	;STI 
...
desinstalar proc near
	...
	MOV ES:[60H*4],AX
	;MOV AX, OLD_1C					;ESTARIA SIN COMENTAR SI LO INSTALARA BIEN
	;MOV ES:[1CH*4],AX
	;MOV AX, OLD_1C+2
	;MOV ES:[1CH*4+2],AX
	STI
